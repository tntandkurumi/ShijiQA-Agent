...
功能：
    读取典故事件数据库 CSV 文件（包含“标题”“内容”“例子”三列），
    使用 Qwen3-Embedding-8B 模型对“标题”列进行嵌入，
    将嵌入向量、标题（作为文档）以及内容、例子（作为元数据）存入 ChromaDB 持久化集合。

数据文件：
    /root/autodl-tmp/毕业论文/数据库6-典故事件数据库/典故事件筛选结果.csv

模型路径：
    /root/autodl-tmp/Qwen3-Embedding-8B

输出：
    持久化 ChromaDB 数据库，存储于 /root/autodl-tmp/毕业论文/chromadb_event
    集合名称：event_info
...
# -------------------- 配置 --------------------
CSV_PATH = "/root/autodl-tmp/毕业论文/数据库6-典故事件数据集/典故事件筛选结果.csv"
MODEL_PATH = "/root/autodl-tmp/Qwen3-Embedding-8B"
DB_PATH = "/root/autodl-tmp/毕业论文/chromadb_event"        # 向量数据库持久化路径
COLLECTION_NAME = "event_info"
BATCH_SIZE = 32                     # 每批处理条数，根据显存调整
PROMPT = "为历史典故事件检索任务生成嵌入向量"
...
print(f"成功将 {total_rows} 条典故事件数据的嵌入向量存入ChromaDB集合 '{COLLECTION_NAME}'。")
