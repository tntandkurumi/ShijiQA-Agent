...
功能：
    基于已加载的模型和 ChromaDB 集合，对输入的术语词进行检索。
    优先精确匹配（术语词完全一致），返回所有匹配记录；
    若无精确匹配，则向量检索，按相似度距离分组，返回前三个不同距离值组的所有记录。

输入参数：
    model (SentenceTransformer) : 已加载的嵌入模型
    collection (chromadb.Collection) : 已连接的术语句法数据库集合
    query_str (str) : 待检索的术语词
...
def _format_record(metadata: dict, term: str = None) -> str:
    """格式化单条记录（优先使用传入的术语词）"""
    lines = []
    if term is not None:
        lines.append(f"术语词：{term}")
    explanation = metadata.get("解释", "")
    if explanation != "" and explanation is not None:
...
def retrieve(model, collection, query_str: str) -> str:
    ...
    if matched_indices:
        ...
        for idx in matched_indices:
            results.append(_format_record(all_metas[idx], term=all_docs[idx]))
        return "\n\n".join(results)
    ...
    for dist in top_groups:
        ...
        for doc, meta in group_records:
            group_output.append(_format_record(meta, term=doc))
        output_parts.append("\n".join(group_output))
        group_index += 1
